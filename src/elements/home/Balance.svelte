<script lang="ts">
  ...
  import Skeleton from "../../elements/misc/Skeleton.svelte";

  let objValue: any;
</script>

<div class="flex flex-col gap-2 items-center mb-6 md:mb-10">
  <h3 class="text-[#ffffff99] uppercase text-xs md:text-base">total balance</h3>
  <div class="flex gap-2 md:gap-4 items-center justify-center">
    ...
    >
      {#if stat.loading}
        <Skeleton
          className="w-44 h-7 sm:w-72 sm:h-10 md:w-96 md:h-14 md:rounded-lg lg:w-[32rem] lg:h-16 lg:rounded-xl"
        />
      {:else}
        {@html animateValue(objValue, stat.balanceCoins)}
      {/if}
    </div>
  </div>
</div>
